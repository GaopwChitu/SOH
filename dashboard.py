...
def run_training_form(default_data: Path, default_out: Path):
    with st.sidebar.form("train_form"):
        ...
        epochs = st.number_input("训练轮数", min_value=1, max_value=5000, value=1000, step=1)
        lr = st.number_input("学习率", min_value=1e-6, max_value=1.0, value=1e-4, format="%.6f")
        weight_decay = st.number_input("权重衰减", min_value=0.0, max_value=1.0, value=1e-5, format="%.6f")
...
def main():
    ...
    default_data = Path("dataset/B05_discharge_soh.csv ")
