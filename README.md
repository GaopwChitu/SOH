...
## 训练/评估
```bash
python soh_model.py --data-path dataset  --output-dir outputs  --window 8  --epochs 30  --lr 1e-3  --weight-decay 1e-4  --batch-size 32  --train-ratio 0.7  --val-ratio 0.1
```
参数说明：
...
```
侧栏可输入特征/模型/指标文件路径（默认指向 `outputs`），并选择电芯。侧栏显示 test_* 和 val_* 指标（如有）。
...
- `logs/`：每次训练生成 `run_*.json`，记录时间戳、参数与指标，便于历史回溯。
